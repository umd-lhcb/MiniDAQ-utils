...
void runPRBSTest( string dpe, bool value ) {
  ...
  if( value ) { // start the test
    ...
    dpConnect( "EP_updatePRBSTest", "UTSLICETEST:TELL40_Dev1_1.top_tell40_monitoring.memory.readings" );
//    dpConnect( "EP_updatePRBSTest", "UTSLICETEST:TELL40_Dev1_0.top_tell40_monitoring.memory.readings" );
  } else {
    if( dpDisconnect( "EP_updatePRBSTest", "UTSLICETEST:TELL40_Dev1_1.top_tell40_monitoring.memory.readings" ) == -1 ) {
...
//        DebugTN( substr(aux, i+24, 8), substr(aux, i+16, 8), substr(aux, i+8, 8), substr(aux, i, 8));
    // salt asic data on bytes 0 to 11 only... the first 4-bytes are probably the GBT header...
    string elinkData = strjoin( makeDynString( substr(aux, i+16, 8), substr(aux, i+8, 8), substr(aux, i, 8) ), "" );
//     DebugTN( elinkData );
...
//   DebugTN( data );
    ...
    for( int iCycle = 2; iCycle <= dynlen( data[iByte] ); iCycle++ ){
      ...
      if( data[iByte][iCycle] != expectedValue ) {
        if(numberOfErrorsPerByte[iByte] < 100 ){
          DebugTN( "Error on byte " + iByte + " and cycle " + iCycle, data[iByte][iCycle], data[iByte][iCycle-1], expectedValue );
  ...
  string status = "";
  status = secondsSinceStart + "s | " + numberOfFramesProcessed + " | " + strjoin( numberOfErrorsPerByte, "-" );
//   DebugTN( status );
//   TEXT_FIELD_status.text( status );
//   TEXT_FIELD_status.text( (status + "|  Errors | " + strjoin( numberOfErrorsPerByte, "|" )) );
  dpSet( "UTSLICETEST:SaltPRBSTest.secondsSinceStart", secondsSinceStart );
  dpSet( "UTSLICETEST:SaltPRBSTest.nFramesProcessed", numberOfFramesProcessed );
  dpSet( "UTSLICETEST:SaltPRBSTest.nErrorsPerByte", numberOfErrorsPerByte );
